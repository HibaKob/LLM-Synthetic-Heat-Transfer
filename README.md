...
* STAGE 1: **BUILD SEEDS** human authored problems ──▶ build_seeds.py ──▶ verification.py checks the human work ──▶ verified seeds.json 
...
The two stages above break down into the following steps:
...
2. **Generation.** An LLM generates new problems in the same style and difficulty as each seed, varying the scenario and numbers while keeping the problem verifiable.
...
In total, there are 39 Type A problems and 6 Type B problems. Type A problems are analytical problems that ask for computing a numerical solution for a single unknown. For each Type A generated seed, a Python check_expression computes the answer independently, to maintain robustness of the code and diminish possible human errors. Type B problems ask for a Python function that performs a computation; each carries a reference solution and test cases whose expected outputs are computed by executing that solution.
...
In this work, the LLM used is llama3.1. Despite the convenience, the caveat of using such small local models is that the more challenging Type B generation has lower yield.

### Data provenance

Every example records its origin, and only examples that passed deterministic verification are kept:
- `human_authored` — written by the author.
- `llm_generated_verified` — produced by a model.
...
- **Single-teacher bias (planned mitigation).** Multi-teacher generation is on the roadmap to reduce a dataset mimicking one model's quirks.
...
This project generates problems using a local LLM served by Ollama. Install Ollama following the instructions [here](https://ollama.com/download), then ensure the Ollama server is running either via the desktop application or by running the command ``ollama serve`` in a terminal. Finally, pull the LLM model. 
``` bash
ollama pull llama3.1
```

To test if the model works, try: 
```bash 
ollama run llama3.1 "Write one sentence about heat conduction."
```
If that prints a response, Ollama is ready.
...
3. Now, create an editable install of ``LLM-Synthetic-Heat-Transfer``:
```bash
pip install -e .
```
...
```
Note that the 3 dots in ``.../scripts/build_seeds.py`` should be replaced by the path on your machine. 

Running ``build_seeds.py`` will generate a ``heat_transfer_seeds.json`` file inside the ``seeds`` folder contained in the ``data`` folder. 

To have the LLM generator create new problems based on the seeds, run the ``generate.py`` script file found in the ``syntheticheatransfer`` folder contained in the ``src`` folder. 

In a terminal running the ``LLM-Synthetic-Heat-Transfer-env`` environment, run the following python command.
```bash
python .../src/syntheticheatransfer/generate.py --model llama3.1:latest --per-seed 2  
```
Note that the 3 dots in ``.../src/syntheticheatransfer/generate.py`` should be replaced by the path on your machine. Also, the generator and (planned) judge models are set via configuration so they can be swapped or mixed without code changes. The following arguments can be passed:
* ``--seeds``: path to ``heat_transfer_seeds.json``; default is the file that is automatically generated using ``build_seeds.py``.
* ``--out``: path to where generated examples should be saved; default is the path to ``generated_examples.json`` saved in ``data/generated``.
* ``--model``: LLM model to be used; default is llama3.1:latest
* ``--per-seed``: number of new problems to be generated per input seed; default is 2.
