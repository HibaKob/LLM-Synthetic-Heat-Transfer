...
def _create_folder(folder_path: Path, new_folder_name: str) -> Path:
    """Create `new_folder_name` inside `folder_path` and return the new path.
    Creates parent directories as needed and does nothing if it already exists.
    """
...
def _expression_namespace() -> dict:
    ...
    # Also expose common names bare, so 'pi', 'sqrt', etc. work without 'math.'
    ns["pi"] = math.pi
    ns["e"] = math.e
    ns["sqrt"] = math.sqrt
    ns["log"] = math.log
...
def expression_namespace() -> dict:
    """
    Return the namespace used to evaluate Type A check_expressions
    (math, and numpy if available).
    """
    return _expression_namespace()
 
 
def load_solution(source: str, function_name: str) -> Callable:
    """
    Execute a Type B `source` (function definition) in the verification
    sandbox and return the named callable. 
    """
    ns = _safe_exec(source)
    return _get_callable(ns, function_name)


def create_output_folder(folder_path: Path, new_folder_name: str) -> Path: 
    """
    Create new directory to save output.
    """
    new_dir_path = _create_folder(folder_path, new_folder_name)
    return new_dir_path
